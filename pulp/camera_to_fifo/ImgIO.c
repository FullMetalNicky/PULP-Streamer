...
#define CHUNK_SIZE  65536 // 8192 // 8192
...
int PushImageToFifo(int fifo, unsigned int W, unsigned int H, unsigned char *OutBuffer)
{
	...
	for(int i=0;i<steps;i++){
		//progress_bar("Writing image ", i, steps);
		ret+=rt_bridge_write(fifo, OutBuffer +(CHUNK_SIZE*i), CHUNK_SIZE,NULL);
	}
	if(((W*H) % CHUNK_SIZE) != 0)
		ret+=rt_bridge_write(fifo, OutBuffer+(CHUNK_SIZE*steps) , ((W*H) % CHUNK_SIZE)*sizeof(unsigned char),NULL);

	// printf("T1: %d (%d)\n", rt_time_get_us() - a, ret);
